...
main()
{
    ...
    for(i=0; i<n; i++)
    {
        printf(" %s  %d %lf%d \n", nbk[kkat[i]].onoma, nbk[kkat[i]].ypol, nbk[kkat[i]].epit, nbk[kkat[i]].imer);
    }
    printf("\n");
...
int rin(FILE *f1, trl kat[], int *synyp, int *mka)
{
    ...
    while (TRUE)
    {
        nscan = fscanf(f1, "%40[^,], %d, %lf, %d%c",
                        name, &ypoloipo, &epitokio, &imerominia, &termch);
...
void fillIn(trl kat1[], trl kat2[], FILE *f2, int a, int b)
{
     ...
     for(i=0; i<a; i++)
    {
        if (kat1[i].imer >2000)
             fprintf(f2, " %d  %lf %d \n",
                     kat1[i].onoma, kat1[i].ypol, kat1[i].epit, kat1[i].imer);
      ...
      for(i=0; i<b; i++)
    {
        if (kat2[i].imer >2000)
             fprintf(f2," %s  %lf %d \n",
                     kat2[i].onoma, kat2[i].ypol, kat2[i].epit, kat2[i].imer);
