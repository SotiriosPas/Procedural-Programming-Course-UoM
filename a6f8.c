...
printf("------------------------------------------------------------------------\n");
for (i=0;i<pl;i++)
printf("%-20s %-15s %-30s %-d\n",patient[i].epwnimo,patient[i].onoma,patient[i].address,patient[i].price);
}
...
printf("%-20s %-15s %-30s %-s\n","EPWNYMO","ONOMA","DIEFTHYNSH","POSO");
printf("------------------------------------------------------------------------\n");
printf("%-20s %-15s %-30s %-d\n",patient[asthenis].epwnimo,patient[asthenis].onoma,patient[asthenis].address,patient[asthenis].price);
}
...
void search(profile patient[],int pl)
{
    ...
    for (i=0;i<pl;i++)
    {
        if ((strcmp(patient[i].epwnimo,givenlastname)==0) && (strcmp(patient[i].onoma,givenfirstname))==0)
        {
        found=TRUE;
        printone(patient,i);
        break;
